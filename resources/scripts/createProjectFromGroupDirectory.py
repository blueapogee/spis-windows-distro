...
groupsDirectory=File("/home/benj/Bureau/testpython/groups")

if groupsDirectory.isDirectory():
    
    groupFiles=groupsDirectory.listFiles()
    
    for groupFile in groupFiles:

        # Creates the Project Data Transfer Object
        projectDTO = Project()
        projectDTO.setProjectName("DefaultProject"+groupFile.getName())
        ...
        studyDTO = Study()
        studyDTO.setStudyName("DefaultStudy"+groupFile.getName())
    ...
    print"All simulations build from several groups are finished"

else:
   print "the group directory given in input is not a directory"
