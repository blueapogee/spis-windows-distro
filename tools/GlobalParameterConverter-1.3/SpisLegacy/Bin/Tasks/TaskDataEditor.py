...
class DataEditor:
    def __init__(self, frame, data_to_edit):
        ...
        self.addBut    = pawt.swing.JButton('Add',
                                            actionPerformed = self.addGroup)
        self.removeBut = pawt.swing.JButton('Remove',
                                            actionPerformed = self.removeGroup)
    ...
    def setCurrentValue(self, instance):
        self.data_to_edit.Value = instance.getText()
...
class TaskDataEditor(Task):
    desc = "Data Editor"
    def run_task(self):
        editor = DataEditor(
            create_internal_frame("DataEditor",
                                  sharedFrames["desktop_pane"]))
        editor.frame.reshape(0, 0, 578, 350)
        editor.show()
        print "task DataEditor is daemonic"
