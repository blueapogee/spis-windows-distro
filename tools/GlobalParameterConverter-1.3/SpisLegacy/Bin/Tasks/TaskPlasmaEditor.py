...
class PlasmaEditor(TemplateEditor):
        
    def __init__(self, frame):
        self.templateID = "defaultPlasmaList"
        self.templateIDListType = "PlasmaList"
        self.templateIDType = "Plasma"
        self.templateIDListName = "PlaDataList"
        self.propertiesTypeList = None
        self.relatedPropList = None
        TemplateEditor.__init__(self, frame)

        
class TaskPlasmaEditor(Task):
    desc = "Plasma Editor"
    
    def run_task(self):
        frame = create_internal_frame("Plasma Models Editor",sharedFrames["gui"].getCurrentDesktop())
        self.size = frame.getParent().getSize()
        frame.reshape( 0, 0, (self.size.width)/3, self.size.height)
        editor = PlasmaEditor(frame)
        editor.show()
        print "task PlasmaEditor is daemonic"
