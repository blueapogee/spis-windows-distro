...
class ProjectWriter3(ProjectWriter):
    """Project saving task"""
    desc = "Saving of the current project"
    
    def write(self):
        
        import Bin.ProjectWriter
        from Bin.ProjectWriter import ProjectWriter
        reload(Bin.ProjectWriter)
        from Bin.ProjectWriter import ProjectWriter
        ...
        if sharedProjectInfos != None:
            ...
            subdir = os.path.join(self.dirName, "infos")
        ...
        if sharedProp != None:
            ...
            #try:
            ioManager.saveMaterialListToDir(sharedProp['defaultMaterialList'].List, os.path.join(self.dirName, "Properties", "Materials"))
            ...
            try:
                ioManager.saveElectricalNodeListToDir( sharedProp['defaultElecNodeList'].List, os.path.join(self.dirName, "Properties", "ElecNodes"))
            except:
                 print "No elecNode to save"
        
            print "Export of plasmas as Jython modules"
            try:
                ioManager.saveElectricalNodeListToDir(sharedProp['defaultPlasmaList'].List, os.path.join(self.dirName, "Properties", "Plasmas"))
            except:
        ...
        if sharedGroups != None:
            subdir = os.path.join(self.dirName, "inputs", "groups")
            ioManager.createDirectory(subdir)
        ...
        if sharedData['AllDataField'] != None:
            ...
            for dataTmp in sharedData['AllDataField'].List:
              ...
              fileNameOut = os.path.join(self.dirName, "DataFields", tmpName)
        ...
        subdir = os.path.join(self.dirName, "inputs", "sim_param")
        ioManager.createDirectory(subdir)
        ...
        try:
            subdir = os.path.join(self.dirName, "inputs", "geo")
            ioManager.createDirectory(subdir)
        ...
        subdir = os.path.join(self.dirName, "inputs", "links")
        ioManager.createDirectory(subdir)
        ...
        if ( fileList != []):
           for file in fileList:
               subdir = os.path.join(self.dirName, "outputs", "vtk")
               ioManager.createDirectory(subdir)
        ...
        for fileName in fileList:
            ...
            if (len(splittedName) > 1):
                if (splittedName[1] == "log"):
                    subdir = os.path.join(self.dirName, "logs")
                    ioManager.createDirectory(subdir)
