"""
**Module Name:**  EdgeList

**Project ref:**  Spis/SpisUI

**File name:**    EdgeList.py
...
class EdgeList(PickledList):
    ...
    def Add_Edge(self, Edge):
        ...
        self.IdList.append(Edge.Id)
        self.NbEdge = self.NbEdge+1

    def Del_Edge(self, Edge):
        if Edge.Id not in self.IdList:
            print ' Edge is not in EdgeList'
        else:
            i = self.IdList.index(Edge.Id)
            del self.List[i]
            del self.IdList[i]
            self.NbEdge = self.NbEdge-1
